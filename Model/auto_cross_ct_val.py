...
# --------------------- 外部工具：从 epoch_metrics 选择各类“最佳” ---------------------
def _select_best_epochs(metrics_tsv: str) -> Dict[str, Optional[int]]:
    """
    返回各指标对应的最佳 epoch（1-based）。若文件缺失或列缺失，返回 None。
    keys: 'auc', 'f1', 'valloss', 'avg' (avg = (auc+f1)/2)
    """
...
def _resolve_checkpoint_for_epoch(outs_dir: str, epoch: int) -> Optional[str]:
    """
    在 outs_dir 下查找该 epoch 的权重文件（训练阶段保存的最后20个：model_epoch_XXX.pth）。
    找不到返回 None。
    """
    if epoch is None:
        return None
...
def export_multi_best_models(session_dir: str,
    ...
    """
    对 session_dir 下每个癌症类型子目录，导出多份 best model：
      - best_by_auc.pth, best_by_f1.pth, best_by_valloss.pth, best_by_avg.pth
    说明：
      - F1 对应的 best 若 `best_model.pth` 存在则优先用它；否则回落到对应 epoch 的快照。
      - 其余指标依赖训练阶段已保存的 per-epoch 快照（本方案为“最后20个epoch”）。
      - 若最佳 epoch 不在已保存快照中，将在 `export_warnings.txt` 记录提示，不报错。
    """
    warn_lines = []
    ...
    if warn_lines:
        with open(os.path.join(session_dir, "export_warnings.txt"), "w") as wf:
            wf.write("\n".join(warn_lines))
        print("⚠️ 部分最佳epoch未找到对应快照，详情见 export_warnings.txt")
    else:
        print("✅ 多指标 best models 导出完成。")


# --------------------- 外部工具：基于多个best模型的 t-SNE ---------------------
@torch.no_grad()
def _collect_logits(model: nn.Module,
                    loader: DataLoader,
                    device: torch.device) -> Tuple[np.ndarray, np.ndarray, List[str]]:
    """
    返回：logits: [N], labels: [N], filenames: list[str]
    使用模型直接前向，取最终 logit（不做sigmoid）。适配你现有 Dataset+collate。
    """
    model.eval()
    ...
    for branches, labels, names in loader:
        ...
        logits = model(branches)  # 期望输出 shape [B, 1] 或 [B]
        if logits.ndim == 2 and logits.shape[1] == 1:
            logits = logits.squeeze(1)
        elif logits.ndim != 1:
            logits = logits.view(logits.shape[0], -1).mean(1)  # 兜底
...
def build_tsne_for_multi_best(session_dir: str,
    ...
    """
    对每个癌症类型的验证集，用导出的多个 best 模型（若存在）分别取 logit，拼为 [N × M]，做 t-SNE。
    产物（写在各自 outs_dir 下）：
      - tsne_coords.tsv
      - tsne_params.json
      - tsne_scatter.png
    依赖：对应 outs_dir 下存在 best_by_*.pth；若某些不存在，则降维时仅使用存在的那几类模型。
    """
    # 复用 Dataset 配置（与训练一致）
    dataset = TSVDataset(data_dir)
    if len(dataset) == 0:
        print("未找到有效的样本！"); return
    ...
    for ct in cancer_types:
        ...
        if not existing:
            print(f"[{ct}] 没有可用的 best_by_* 模型，跳过 t-SNE。")
            continue

        # 划分验证集（与训练一致：该 ct 作为 val）
        ...
        plt.close()
        print(f"[{ct}] t-SNE 完成：{outs_dir}/tsne_coords.tsv, tsne_scatter.png")
...
def gpu_mem_info_mb():
    """
    返回 (free_mb, total_mb) 若 CUDA 可用；否则返回 (None, None)
    """
    if not torch.cuda.is_available():
        return (None, None)
    try:
...
def auto_cross_val(data_dir,
    ...
    # 配置要保存的尾部 epoch 数
    save_last_k = 30  # 保存最后 20 个 epoch 的权重
    ...
    for val_cancer_type in cancer_types:
        ...
        for epoch in pbar:
            ...
            # ***** 仅改“最优选择”：由 AUC→F1（AUC 仅作并列时的次级比较） *****
        ...
        # ==== 保存最优模型并评估（Best by F1） ====
        assert best_state is not None, "未产生最优状态，请检查训练/验证流程。"
        ...
        val_loss, val_acc, val_prec, val_rec, val_f1, val_auc, val_cm, gt, probs, _ = \
            evaluate_detailed(model, val_loader, criterion, device)
        print(f"\n*** 验证集 {val_cancer_type} 评估（Best@Epoch {best_epoch} by F1）***")
        print(
        ...
        # ==== 附：在 timings.tsv 末尾追加 TOTAL 行（统计用时） ====
        with open(timings_tsv, "a") as f:
...
if __name__ == '__main__':
    ...
    # 2) 生成 k 个随机种子
    seeds = [530512]  # 你也可以改回 np.random.randint(...)
    for se in seeds:
        print(f"\n===== Run with seed {se} =====")
        if mut_type == "amp":
        ...
        with open(os.path.join(out_dir, "hyperparameters.txt"), 'a') as f:
            f.write(f"seed: {se}\n")
    print("✅ 所有 runs 完成，并已生成")

    # —— 训练全部结束后：对每个 session_dir 做多指标 best 导出与 t-SNE ——
    for sd in session_dirs:
        print(f"\n=== 导出多指标 best models @ {sd} ===")
        export_multi_best_models(sd, criteria=('auc', 'f1', 'valloss', 'avg'))

        print(f"=== 构建基于多 best 的 t-SNE @ {sd} ===")
